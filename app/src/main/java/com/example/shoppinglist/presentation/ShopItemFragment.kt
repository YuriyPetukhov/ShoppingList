...
// Метод, которы вызывается в момент прикреплетия Fragment к Activity, context - Activity, к которой он прикрепляется
    ...
    //Получаем параметры из Intent, переданного из MainActivity в ShopItemActivity и отправленные в Bundl'е в ShopItemFragment (то есть сюда)
    //Проверки содержимых, по большей части для и от других разрабов
    ...
    companion object {
        ...
        //        Заворачивание аргументов фрагмента в Bundle, т.к при пересоздании фрагмента у него вызывается пустой конструктор
        //Написано в облегченном Kotlin-стиле
        ...
        //Написано в полноценном Kotlin-стиле, промежуточные переменные не создаются, просто вызываются два .apply
